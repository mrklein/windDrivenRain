...
// Interpolation for getCdRe
inline Foam::scalar interpolate_CdRe(Foam::label i, Foam::scalar v,
                                     const Foam::scalarList& f,
                                     const Foam::scalarList& x) {
  const auto df = f[i + 1] - f[i];
  const auto dx = x[i + 1] - x[i];
  const auto dv = v - x[i];
  return (f[i] + df / dx * dv) * v;
}
...
Foam::tmp<Foam::volScalarField> ethz::getCdRe(
  ...
  forAll(Reynolds, celli) {
    ...
    if (Rei < min(Re_M)) {
      (*CdRe)[celli] = interpolate_CdRe(0, Rei, Cd_M, Re_M);
    } else if (Rei >= max(Re_M)) {
      (*CdRe)[celli] = interpolate_CdRe(32, Rei, Cd_M, Re_M);
    } else {
      forAll(Re_M, i) {
        if (not in(i, Rei, Re_M)) continue;
        (*CdRe)[celli] = interpolate_CdRe(i, Rei, Cd_M, Re_M);
        break;
      }
    }
  }
